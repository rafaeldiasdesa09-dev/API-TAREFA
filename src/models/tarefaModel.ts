...
export async function criar(dados: { titulo: string; descricao?: string;

prioridade: "alta"|"media"|"baixa" }): Promise<Tarefa> {

const tarefas = await carregar();

const nova: Tarefa = { id: (tarefas.at(-1)?.id ?? 0)+1, ...dados,
